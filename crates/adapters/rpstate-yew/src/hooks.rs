...
use rpstate::client::{AsyncSubscriptionBackend, Field, ReactiveMap};
use rpstate::core::primitives::map_core::{ReactiveMapKey, ReactiveMapValue};
use rpstate::reactive::FieldValue;
use rpstate::MapChange;
...
#[hook]
pub fn use_rpstate<S>() -> S
where
    S: Clone + PartialEq + 'static,
{
    use_context::<S>().unwrap_or_else(|| {
        panic!(
            "rpstate-yew: State slice '{}' not found in context. \
             Did you render RpStateProvider and include it in provide_slices!?",
            std::any::type_name::<S>()
        )
    })
}
...
use rpstate_arena::{DefaultArena, FieldHandle, MapHandle, RpStateFrameworkNested, WritableHandle, WritableMapHandle, PIPELINE_ARENA};
use serde::de::DeserializeOwned;
use serde::{Deserialize, Serialize};
use std::collections::HashMap;
//TODO: use subcription_external
...
where
    S: rpstate_arena::RpStateFrameworkNested + 'static,
{
...
pub fn use_rpstate<S>() -> S::Handle
where
    S: rpstate_arena::RpStateFramework<crate::LeptosBackend> + 'static,
{
    if let Some(handle) = use_context::<S::Handle>() {
        return handle;
    }

    let store = use_context::<rpstate::DefaultStore>().unwrap_or_else(|| {
        panic!(
            "rpstate-leptos: DefaultStore not found in context while trying to initialize '{}'. \
    ...
    let arena = use_context::<DefaultArena>().unwrap_or_else(|| {
        panic!(
            "rpstate-leptos: DefaultArena not found in context while trying to initialize '{}'. \
             Make sure RpStateProvider is rendered at the root of your application.",
            std::any::type_name::<S>()
        );
    });
    ...
    let state = S::load_slice(&store).unwrap_or_else(|err| {
        panic!(
            "rpstate-leptos: Failed to load state slice '{}': {err}",
            std::any::type_name::<S>()
        );
    });
    ...
    M: AccessMode,
{
    ...
    let key_clone = key.clone();
    let sub = arena.subscribe_map_key(handle, key_clone, move |change: &MapChange<_, V>| match change {

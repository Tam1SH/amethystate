...
"#,
    ...
    pub fn export_rust(&self, out_path: impl AsRef<Path>) -> std::io::Result<()> {
        let mut code = String::new();
        code.push_str("// GENERATED AUTOMATICALLY. DO NOT EDIT.\n");
        code.push_str("use tauri_plugin_rpstate::client::field::Field;\n");
        code.push_str("use tauri_plugin_rpstate::client::map::ReactiveMap;\n");
        code.push_str("use tauri_plugin_rpstate::client::{invoke_get_prefix, invoke_flush};\n");
        code.push_str("use serde::{Serialize, Deserialize};\n\n");

        for entry in self.registry.values() {
            if let Some(prefix) = entry.prefix {
                code.push_str("#[allow(dead_code)]\n");
                code.push_str("#[derive(Clone, Debug, PartialEq, Eq)]\n");
                code.push_str(&format!("pub struct {} {{\n", entry.struct_name));
                for field in entry.fields {
                    let prop_name = field.name;
                    let prop_type = match &field.kind {
                        FieldKind::Plain | FieldKind::Volatile | FieldKind::Lookup { .. } => {
                            format!("Field<{}>", field.rust_type)
                        }
                        FieldKind::Nested { struct_name }
                        | FieldKind::LookupNode { struct_name, .. } => {
                            format!("{}Fields", struct_name)
                        ...
                        FieldKind::ReactiveMap {
                            ...
                            format!("ReactiveMap<{}, {}>", key_rust_type, value_rust_type)
                        }
                    };
                ...
                code.push_str("    pub async fn load() -> Result<Self, String> {\n");
                code.push_str(&format!(
                    "        let initial = invoke_get_prefix(\"{}\").await?;\n",
                    prefix
                ));
                ...
                for field in entry.fields {
                    ...
                    match &field.kind {
                        FieldKind::Plain | FieldKind::Volatile => {
                            code.push_str(&format!(
                                "            {}: Field::new(\"{}\", initial.get(\"{}\").and_then(|v| tauri_plugin_rpstate::serde_json::from_value::<{}>((*v).clone()).ok()).unwrap_or_default()),\n",
                                prop_name, full_key, full_key, field.rust_type
                            ));
                        }
                        FieldKind::Nested { struct_name } => {
                            code.push_str(&format!(
                        ...
                        FieldKind::Lookup { target_key, .. } => {
                            code.push_str(&format!(
                                "            {}: Field::new(\"{}\", initial.get(\"{}\").and_then(|v| tauri_plugin_rpstate::serde_json::from_value::<{}>((*v).clone()).ok()).unwrap_or_default()),\n",
                                prop_name, target_key, target_key, field.rust_type
                            ));
                        }
                        FieldKind::LookupNode {
                            target_prefix,
                ...
                for (k, v) in &initial {{
                    if let Some(sub_key) = k.strip_prefix(map_prefix) {{
                        if let Ok(parsed_k) = <{2} as std::str::FromStr>::from_str(sub_key) {{
                            if let Ok(parsed_v) = tauri_plugin_rpstate::serde_json::from_value::<{3}>((*v).clone()) {{
                                map_init.insert(parsed_k, parsed_v);
                            }}
                        }}
                    }}
                }}
                ReactiveMap::new(\"{1}\", map_init)
            }},\n",
                                prop_name, full_key, key_rust_type, value_rust_type
                            ));
                        }
                    }
                }
                ...
                code.push_str("    pub async fn save(&self) -> Result<(), String> {\n");
                code.push_str(&format!("        invoke_flush(\"{}\").await\n", prefix));
                code.push_str("    }\n");
                code.push_str("}\n\n");
            } else {
                code.push_str("#[allow(dead_code)]\n");
                code.push_str("#[derive(Clone, Debug, PartialEq, Eq)]\n");
                code.push_str(&format!("pub struct {}Fields {{\n", entry.struct_name));
                for field in entry.fields {
                    let prop_name = field.name;
                    let prop_type = match &field.kind {
                        FieldKind::Plain | FieldKind::Volatile | FieldKind::Lookup { .. } => {
                ...
                code.push_str("#[allow(dead_code)]\n");
                code.push_str(&format!("impl {}Fields {{\n", entry.struct_name));
                code.push_str("    pub fn new(prefix: &str, initial: &std::collections::HashMap<String, tauri_plugin_rpstate::serde_json::Value>) -> Self {\n");
                code.push_str("        Self {\n");
                for field in entry.fields {
                    let prop_name = field.name;
                    match &field.kind {
                        FieldKind::Plain | FieldKind::Volatile => {
                            code.push_str(&format!(
                                "            {}: Field::new(format!(\"{{}}.{}\", prefix), initial.get(&format!(\"{{}}.{}\", prefix)).and_then(|v| tauri_plugin_rpstate::serde_json::from_value::<{}>((*v).clone()).ok()).unwrap_or_default()),\n",
                                prop_name, field.name, field.name, field.rust_type
                            ));
                        }
                        FieldKind::Nested { struct_name } => {
                            code.push_str(&format!(
                                "            {}: {}Fields::new(&format!(\"{{}}.{}\", prefix), initial),\n",
                                prop_name, struct_name, field.name
                            ));
                        }
                        FieldKind::ReactiveMap {
                            ...
                            code.push_str(&format!(
                                "            {0}: {{
                let mut map_init = std::collections::HashMap::new();
                let map_prefix = format!(\"{{}}.{3}.\", prefix);
                for (k, v) in initial {{
                    if let Some(sub_key) = k.strip_prefix(&map_prefix) {{
                        if let Ok(parsed_k) = <{1} as std::str::FromStr>::from_str(sub_key) {{
                            if let Ok(parsed_v) = tauri_plugin_rpstate::serde_json::from_value::<{2}>((*v).clone()) {{
                                map_init.insert(parsed_k, parsed_v);
                            }}
                        }}
                    }}
                }}
                ReactiveMap::new(format!(\"{{}}.{3}\", prefix), map_init)
            }},\n",
                                prop_name, key_rust_type, value_rust_type, field.name
                            ));
                        }
                        _ => {}
                    }

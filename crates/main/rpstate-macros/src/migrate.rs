...
use crate::rpstate::rpstate_crate_path;
...
pub fn migrate_impl(
    ...
    let crate_name = rpstate_crate_path();
    ...
    let inventory_block = quote! {
        #crate_name::inventory::submit! {
            #crate_name::migration::registry::MigrationStepEntry {
                prefix: <#new_ty as #crate_name::migration::fields::RpStateFields>::PARENT_PREFIX,
                target_version: <#new_ty as #crate_name::migration::fields::RpStateFields>::VERSION,
                dependencies: <#new_ty as #crate_name::migration::fields::RpStateFields>::MIGRATION_DEPS,
                description: #description,
                schema_hash: <#new_ty as #crate_name::migration::fields::RpStateFields>::SCHEMA_HASH,
                fields: <#new_ty as #crate_name::migration::fields::RpStateFields>::FIELDS,
                run: |ctx| {
                    use #crate_name::migration::fields::RpStateFields;
                    use #crate_name::migration::migrate_from::MigrateFrom;

                    let old_data = <#old_ty as RpStateFields>::load_struct(ctx)?;
                    let new_data = <#new_ty as MigrateFrom<#old_ty>>::migrate(old_data, ctx)?;

                    for field in <#old_ty as RpStateFields>::FIELDS {
                        let is_renamed = <#new_ty as MigrateFrom<#old_ty>>::RENAMES
                            .iter()
                            .any(|(old_k, _)| *old_k == field.name);
                        let is_kept = <#new_ty as RpStateFields>::FIELDS
                            .iter()
